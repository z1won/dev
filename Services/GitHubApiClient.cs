...
public sealed class GitHubApiClient(HttpClient httpClient, ApiTelemetry telemetry)
{
    ...
    public async Task<IReadOnlyList<GitHubRepository>> GetRepositoriesAsync(string username, int perPage = 12, CancellationToken cancellationToken = default) =>
        await GetJsonAsync<IReadOnlyList<GitHubRepository>>($"/users/{Uri.EscapeDataString(username)}/repos?sort=updated&per_page={Math.Clamp(perPage, 1, 100)}", "GET /users/{user}/repos", cancellationToken) ?? [];
    ...
    public async Task<GitHubReadme?> GetReadmeAsync(string owner, string repository, CancellationToken cancellationToken = default)
    {
        ...
        if (readme?.Content is null)
            return readme;

        try
        {
            var content = readme.Content.Replace("\n", string.Empty).Replace("\r", string.Empty);
            return readme with { DecodedContent = Encoding.UTF8.GetString(Convert.FromBase64String(content)) };
        }
    ...
    public async Task<GitHubFileContent?> GetFileContentAsync(string owner, string repository, string path, string reference, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            return file with { DecodedContent = Encoding.UTF8.GetString(Convert.FromBase64String(content)) };
        }
        catch (FormatException)
        {
            return file;
    ...
    public async Task<IReadOnlyList<GitHubCommit>> GetCommitsAsync(string owner, string repository, int perPage = 5, CancellationToken cancellationToken = default) =>
        await GetJsonAsync<IReadOnlyList<GitHubCommit>>($"/repos/{Uri.EscapeDataString(owner)}/{Uri.EscapeDataString(repository)}/commits?per_page={Math.Clamp(perPage, 1, 20)}", "GET /repos/{owner}/{repo}/commits", cancellationToken) ?? [];
    ...
    public Task<GitHubCompareResult?> CompareCommitsAsync(string owner, string repository, string baseSha, string headSha, CancellationToken cancellationToken = default) =>
        GetJsonAsync<GitHubCompareResult>($"/repos/{Uri.EscapeDataString(owner)}/{Uri.EscapeDataString(repository)}/compare/{Uri.EscapeDataString(baseSha)}...{Uri.EscapeDataString(headSha)}", "GET /repos/{owner}/{repo}/compare", cancellationToken);
    ...
    public async Task<IReadOnlyList<GitHubPullRequest>> GetPullRequestsAsync(string owner, string repository, int perPage = 5, CancellationToken cancellationToken = default) =>
        await GetJsonAsync<IReadOnlyList<GitHubPullRequest>>($"/repos/{Uri.EscapeDataString(owner)}/{Uri.EscapeDataString(repository)}/pulls?state=all&sort=updated&direction=desc&per_page={Math.Clamp(perPage, 1, 20)}", "GET /repos/{owner}/{repo}/pulls", cancellationToken) ?? [];
    ...
    public async Task<IReadOnlyList<GitHubIssue>> GetIssuesAsync(string owner, string repository, int perPage = 5, CancellationToken cancellationToken = default) =>
        await GetJsonAsync<IReadOnlyList<GitHubIssue>>($"/repos/{Uri.EscapeDataString(owner)}/{Uri.EscapeDataString(repository)}/issues?state=all&sort=updated&direction=desc&per_page={Math.Clamp(perPage, 1, 20)}", "GET /repos/{owner}/{repo}/issues", cancellationToken) ?? [];

    public Task<PullRequestIntelligence?> GetPullRequestIntelligenceAsync(string owner, string repository, int number, CancellationToken cancellationToken = default) =>
        GetJsonAsync<PullRequestIntelligence>($"/repos/{Uri.EscapeDataString(owner)}/{Uri.EscapeDataString(repository)}/pulls/{number}", "GET /repos/{owner}/{repo}/pulls/{number}", cancellationToken);

    public async Task<IReadOnlyList<GitHubPullRequestFile>> GetPullRequestFilesAsync(string owner, string repository, int number, CancellationToken cancellationToken = default) =>
        await GetJsonAsync<IReadOnlyList<GitHubPullRequestFile>>($"/repos/{Uri.EscapeDataString(owner)}/{Uri.EscapeDataString(repository)}/pulls/{number}/files?per_page=100", "GET /repos/{owner}/{repo}/pulls/{number}/files", cancellationToken) ?? [];
    ...
    public async Task<IReadOnlyList<string>> GetPullRequestChangedFilesAsync(string owner, string repository, int number, CancellationToken cancellationToken = default) =>
        (await GetPullRequestFilesAsync(owner, repository, number, cancellationToken))
            .Select(x => x.Filename)
            .Where(x => !string.IsNullOrWhiteSpace(x))
            .Cast<string>()
            .ToArray();

    public async Task<IReadOnlyList<PullRequestReviewInfo>> GetPullRequestReviewsAsync(string owner, string repository, int number, CancellationToken cancellationToken = default) =>
        await GetJsonAsync<IReadOnlyList<PullRequestReviewInfo>>($"/repos/{Uri.EscapeDataString(owner)}/{Uri.EscapeDataString(repository)}/pulls/{number}/reviews", "GET /repos/{owner}/{repo}/pulls/{number}/reviews", cancellationToken) ?? [];
    ...
    public async Task<IReadOnlyList<PullRequestThreadInfo>> GetPullRequestThreadsAsync(string owner, string repository, int number, CancellationToken cancellationToken = default) =>
        await GetJsonAsync<IReadOnlyList<PullRequestThreadInfo>>($"/repos/{Uri.EscapeDataString(owner)}/{Uri.EscapeDataString(repository)}/pulls/{number}/comments", "GET /repos/{owner}/{repo}/pulls/{number}/comments", cancellationToken) ?? [];
    ...
    public async Task<string?> GetPullRequestDiffAsync(string owner, string repository, int number, CancellationToken cancellationToken = default)
    {
        ...
        request.Headers.Accept.Add(new System.Net.Http.Headers.MediaTypeWithQualityHeaderValue("application/vnd.github.v3.diff"));
        ...
        try
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                telemetry.Record("GET", "GET /repos/{owner}/{repo}/pulls/{number}.diff", status, stopwatch.ElapsedMilliseconds, message);
                throw new HttpRequestException(message);
            }

            var text = await response.Content.ReadAsStringAsync(cancellationToken);
            telemetry.Record("GET", "GET /repos/{owner}/{repo}/pulls/{number}.diff", status, stopwatch.ElapsedMilliseconds);
            return text;
        }
        catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
        {
            stopwatch.Stop();
            telemetry.Record("GET", "GET /repos/{owner}/{repo}/pulls/{number}.diff", 408, stopwatch.ElapsedMilliseconds, "Request timed out");
            throw;
        }
    }

    public async Task<string?> GetPullRequestFilePatchAsync(string owner, string repository, int number, string path, CancellationToken cancellationToken = default) =>
        (await GetPullRequestFilesAsync(owner, repository, number, cancellationToken))
            .FirstOrDefault(x => string.Equals(x.Filename, path, StringComparison.Ordinal))?.Patch;

    public async Task<IReadOnlyList<GitHubCheckRun>> GetCheckRunsAsync(string owner, string repository, string sha, CancellationToken cancellationToken = default)
    {
        var response = await GetJsonAsync<GitHubCheckRunResponse>($"/repos/{Uri.EscapeDataString(owner)}/{Uri.EscapeDataString(repository)}/commits/{Uri.EscapeDataString(sha)}/check-runs?per_page=100", "GET /repos/{owner}/{repo}/commits/{sha}/check-runs", cancellationToken);
    ...
    public async Task<(string Label, string Detail)> GetCommitStatusAsync(string owner, string repository, string sha, CancellationToken cancellationToken = default)
    {
        ...
        if (status is null)
            return ("NOT CHECKED", "Commit status is not available.");

        return status.State switch
        {
            "success" => ("PASS", "All checks passed."),
            "failure" => ("FAIL", "One or more checks failed."),
            "pending" => ("PENDING", "Checks are still running."),
            _ => (status.State?.ToUpperInvariant() ?? "UNKNOWN", $"{status.TotalCount} status checks reported.")
        };
    }
...
public sealed record GitHubFileContent(string? Name, string? Path, string? HtmlUrl, string? Content, string? Encoding)
{
    [JsonIgnore] public string? DecodedContent { get; init; }
}
...
public sealed record GitHubCheckRun(string? Name, string? Status, string? Conclusion, string? HtmlUrl, DateTimeOffset? StartedAt, DateTimeOffset? CompletedAt);
public sealed record GitHubCheckRunResponse(IReadOnlyList<GitHubCheckRun>? CheckRuns);
